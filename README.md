...
[`examples/sales_customer_dataset_report.html`](examples/sales_customer_dataset_report.html)
is a real report generated by the app for the prompt *"Summarize the dataset
and highlight data quality issues"* against `sample_data/sales_data.csv`.
Download the file and open it in a browser to view it (GitHub renders `.html`
files as source, not as a page); it includes the full executive summary, the
AI's step-by-step analysis plan, key findings, all generated charts, and
recommendations. Highlights from that run:

- **Dataset**: 500 rows × 28 columns, 0 missing values, 0 duplicate rows.
- **Findings**: an unexpected `"Other"` value in the `Gender` column, and
  outliers in `Gross_Sales`, `Discount_Amount`, and `Profit`.
- **Recommendations**: investigate the unexpected category value and review
  the numeric outliers before drawing conclusions from the data.
